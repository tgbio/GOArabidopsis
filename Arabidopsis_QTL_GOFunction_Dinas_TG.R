...
#rel_ids = lapply(at_ids, function(x) length(x[x<0])at_ids %in% mapped.list[keys(all.at.genes)])#]#(all_pos_list$pos_id %in% unbias_cg_adjusted.significant$pos_id), all_pos_list$meth_perc, mapped.list[n], n in keys(all.at.genes)
#rel_ids = at_ids %in% lapply(at_ids, function(x) length(x[x<0])at_ids %in% mapped.list[keys(all.at.genes)])#]#(all_pos_list$pos_id %in% unbias_cg_adjusted.significant$pos_id), all_pos_list$meth_perc, mapped.list[n], n in keys(all.at.genes)
...
new_ids = ""
for (i in 1:length(mapped.list)){ #for variable within the length range of mapped.list
  mapped.list[i]
  print(mapped.list[i])
  if (at_ids[i] %in% mapped)
}
"836900" %in% mapped.list["AT5G67640"]
